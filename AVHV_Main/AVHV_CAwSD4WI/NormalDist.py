...
class NormalDist:
    ...
    def generate_concat_values(self, df_obj, fp, fn):
        ...
        def dist1(x):
            return round(1 / (self.std1 * np.sqrt(2 * np.pi)) * np.exp(-(x - self.mean1) ** 2 / (2 * self.std1 ** 2)), 4)

        def dist2(x):
            return round(1 / (self.std2 * np.sqrt(2 * np.pi)) * np.exp(-(x - self.mean2) ** 2 / (2 * self.std2 ** 2)), 4)
        ...
        df_dist_safe_dist = pd.DataFrame({'distrib_safe_distance': dist_safe_dist_list})
        ...
        ax.set_title('Normal Distribution of Safe Distance ' + '(' + prefix + ')', fontsize=20, pad=20)
        ...
        # legend
        ax.legend((p1[0],), ('Safe Distance Normal Distribution Values',), fontsize=20)
        ...
        for item in ['stopping_time', 'speed']:
            if item == 'stopping_time':
                ax.scatter(ax_num_mean, df_obj_mean['stopping_time'], c='r', label="Stopping Time")
            else:
                ax.scatter(ax_num_mean, df_obj_mean['speed'], c='b', label='Speed')

        ax.set_title('Scatter Plot of Stopping Time and Speed ' + '(' + prefix + ') - Averages of each car\'s values',
                     fontsize=20, pad=20)
        ...
        plt.yticks(np.arange(0, max(df_obj_mean['stopping_time'].max(), df_obj_mean['speed'].max()) + 1, 1))
        ...
        axs[0, 0].hist(df_combined['safe_distance'], 40, facecolor=color, alpha=0.75, density=True)
        axs[0, 0].set_title("Safe Distance", size=20, pad=20)

        axs[0, 1].hist(df_combined['speed'], 40, facecolor=color, alpha=0.75, density=True)
        axs[0, 1].set_title("Speed", size=20, pad=20)

        axs[0, 2].hist(df_combined['deceleration_force'], 40, facecolor=color, alpha=0.75, density=True)
        axs[0, 2].set_title("Deceleration Force", size=20, pad=20)

        axs[1, 0].hist(df_combined['reaction_time'], 40, facecolor=color, alpha=0.75, density=True)
        axs[1, 0].set_title("Reaction Time", size=20, pad=20)

        axs[1, 1].hist(df_combined['stopping_time'], 40, facecolor=color, alpha=0.75, density=True)
        ...
        # x-axis labels
        plt.xticks(np.arange(0, len(df_combined['reaction_time']), 1000), fontsize=16)
        ...
        # plt.xlim(xmin=0)
        # plt.ylim(ymin=0)
        plt.savefig(fp + fn + 'histograms' + '.png')
        plt.close()
