...
class Simulation:
    ...
    def __start_simulation(self):
        ...
        print("\nAVHV Control with CAwS4WI - Please wait for a while...")
        ...
        while self.current_time < self.end_time:
            ...
            self.car_density.append(round(len(self.environment.environment_objects[Car]) * 2.5 / (
                        2.4 * 0.6214), 2))
            ...
            self.car_speed.append(round(
                sum([car.get_speed() * 2.237 for car in
                     self.environment.environment_objects[Car]])/
                len(self.environment.environment_objects[Car]), 2))

            safe_distances = [car.safe_distance for car in self.environment.environment_objects[Car] if 'Aggressive' in car.name]
            ...
            self.safe_distances.append(round(sum(safe_distances) / len(safe_distances),
                                             2))
