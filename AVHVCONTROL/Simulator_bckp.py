...
# from AVHVCONTROL.test.Test2 import car
...
class _TrafficNode:
    ...
    def check(self, node):
        if isinstance(node, _TrafficNode):
...
class _TrafficSystem:
    ...
    def node(self, name):
        if isinstance(name, _TrafficNode):
            return name
        ...
        for node, destinations in edges.items():
            ...
            if isinstance(node, _TrafficNode):
                for destination in destinations:
                    ...
                    if isinstance(destination, _TrafficNode):
...
class Pedestrian(_EnvironmentObject):
    ...
    def update(self, t):
        self.pedestrian_node()
        if len(self.route) > 0:
            self.button_press(t)
            # Obey Traffic Lights
            self.obey_pedestrian_light(t)
            super(pedestrian, self).update(t)
...
class TrafficLight(_EnvironmentObject):
    ...
    def update(self, t):
        super(TrafficLight, self).update(t)
        self.change(t)
...
class CarSpawner(_EnvironmentObject):  # new car arrivals to grow the queue length
    def __init__(self, name, node, direction, route=None, safe_distance=None):
        print('__init function')
