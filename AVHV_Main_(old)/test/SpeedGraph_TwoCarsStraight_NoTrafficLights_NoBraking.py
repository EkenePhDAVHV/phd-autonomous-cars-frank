...
simulation = Simulation(
    ...
    environment=Environment(
        ...
        Car(name="Car 1", route=[1, 2, 4, 5], direction=0, easy_physics=True, velocity=0, velocity_max=max_velocity,
            acceleration=0, acceleration_max=max_acceleration, position=[0, 0], no_braking=True),
        Car(name="Car 2", route=[1, 2, 4, 5], direction=0, easy_physics=True, velocity=0, velocity_max=max_velocity + 10,
            acceleration=0, acceleration_max=max_acceleration, position=[0, 0], no_braking=True, idle_time=10)
    ])
)
...
ax.set(xlabel='Time (s)', ylabel='Speed (m/s)', title='2 Cars in a straight line with delay for second car')
ax.plot(time, speed)
