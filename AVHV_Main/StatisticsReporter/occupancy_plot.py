...
def plot_occupancy_matrix(obj, x_vals, y_vals, title, x_label, y_label, all_fp,
    ...
    try:
        ...
        #     fontsize=58, pad=50, fontweight='bold')
    ...
    # x and y-axes labels
    plt.xticks(np.arange(max(x_vals), min(x_vals) - 1, -5), fontsize=34)
    plt.yticks(np.arange(min([min(y_vals[0]),
                              min(y_vals[1]),
                              min(y_vals[2])]) - 0.1,
                         max([max(y_vals[0]),
                              max(y_vals[1]),
                              max(y_vals[2])]) + 1.0, 0.1), fontsize=34)
