...
class PhysicalObject:
    def _init_(self, pos=[0, 0], mass=1020, max_acceleration=1, drag=1.0,
               max_deacceleration=1, acceleration=[0, 0], velocity=[0, 0]):
    ...
    def get_speed(self):
        ...
        return math.sqrt(
            self.velocity[0] * self.velocity[0] + self.velocity[1] *
            self.velocity[1])

    def get_force(self):
        return math.sqrt(
            self.acceleration[0] * self.acceleration[0] + self.acceleration[1] *
            self.velocity[1]) * self.mass

    def _str_(self):
        return "pos: %s mass:%s maxAccl:%s maxDeaccl:%s Accl:%s Velocity:%s" % (
        self.pos, self.mass, self.max_acceleration, self.max_deacceleration,
        self.acceleration, self.velocity)


class Physics:
    def _init_(self, friction=0.75, gravity=9.8):
...
class CurveMovement():
    def _init_(self, time, car, radius, curveCenter, startDegree, endDegree):
    ...
    def move(self, car, time):
        reachedEnd = False
        # degree = 360 * car.get_speed() / self.lengthCircle

        vel = car.get_speed().get_value()
        car_speed = math.sqrt(vel[0] * vel[0] + vel[1] * vel[1])
        car_pos = car.position.get_value()

        degree = 360 * car_speed / self.lengthCircle
        ...
        if reachedEnd:
            ...
            timeAfterEndingCurve = (
                                               self.actualDegree - self.endDegree) / 360 * self.lengthCircle / car_speed
        ...
        car_pos[0] = self.curveCenter[0] + self.radius * math.sin(alpha)
        car_pos[1] = self.curveCenter[1] + self.radius * math.cos(alpha)
...
class PhyicsTest(unittest.TestCase):
    ...
    def testCurveRight(self):
        ...
        c = PhysicalObject(pos=[0, 3], velocity=[max_speed_curve, 0],
                           acceleration=[0, 0])
