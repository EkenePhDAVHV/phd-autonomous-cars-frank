...
class Vector2:
    """The blueprint class for vector objects in the simulation."""
    ...
    def scale_self(self, scale_value):
        vector = self.scale(scale_value)
        self.x = vector.x
        self.y = vector.y
        return self

    def round_to(self, decimal_places):
        vector = self.copy()
        vector.x = round(vector.x, decimal_places)
        vector.y = round(vector.y, decimal_places)
        return vector

    def round_to_self(self, decimal_places):
        vector = self.round_to(decimal_places)
        self.x = vector.x
        self.y = vector.y
        return self

    def direction(self, vector=None):
        """Return direction of this vector to the vector supplied;
        if no vector is supplied, then from origin to this vector.
          ...
          Returns:
            direction: float
              The direction from this vector object to the vector object
              supplied.
        """

        return math.atan2(vector.y - self.y, vector.x - self.x) \
            if isinstance(vector, Vector2) else math.atan2(self.y, self.x)
